...
data <- data.table::fread(
  ...
  arrange(desc(env_breadth)) %>%
  print()

# Remove the upper outlier
env_breadth_threshold <- quantile(data$env_breadth, 0.995)
data <- data %>%
  filter(env_breadth <= env_breadth_threshold)
...
# Number of species: 263 species
unique(data$species) %>% length(.)
# Number of tropical species: 183 species
unique(data_tropical$species) %>% length(.)
# Number of nontropical species: 80 species
...
# NOTE: Below not needed as there are no negative values, but I'm keeping the code
