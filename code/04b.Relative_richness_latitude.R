...
save(
  ...
  file = "output/generated_data/figure_2b.RData"
)

#### * Absolute and relative richness latitudinal gradients * ####

# Create the plot: Relative richness and absolute richness vs latitude

# Read and prepare the data
fitted_values_absolute <- data.table::fread(
  "data/presence/richness_fitted_absolute.txt", sep = "\t"
)

# Compute max upper limit
max_upper <- fitted_values_absolute %>%
  pull(upper_poly) %>%
  max(na.rm = TRUE)
...
# Combine datasets and apply scaling
fitted_ploynomials_combined <- fitted_values_absolute %>%
  inner_join(
    fitted_values_combined %>% select(
      model, observed, fitted, fitted_upper, fitted_lower, longitude, latitude
    ),
    by = c("model", "longitude", "latitude")
  ) %>%
  mutate(
    fitted_absolute_scaled = min_max_scale(fitted_poly),
    lower_absolute_scaled = min_max_scale(lower_poly),
    upper_absolute_scaled = min_max_scale(upper_poly)
  )

# Plot relative and absolute richness relationship with latitude
plot_latitude_relationship_sup <- ggplot(fitted_ploynomials_combined, aes(x = latitude)) +
  # Relative Richness Line and Ribbon
  geom_ribbon(aes(ymin = fitted_upper, ymax = fitted_lower), fill = "#de2d26", alpha = 0.2) +
  geom_line(aes(y = fitted), colour = "#de2d26", linewidth = 1) +
  
  # Absolute Richness Line and Ribbon (Using Scaled Values)
  ...
  # Y-axis Scaling
  scale_y_continuous(
    name = "Richness (log<sub>10</sub>-ratio)",
    limits = c(-2.2, 2), 
    breaks = c(-2, -1, 0, 1, 2),
    sec.axis = sec_axis(
      transform = ~ ((. - (-2.2)) / (2 - (-2.2))) * (max_upper - 0) + 0,  # Reverse min-max scaling
      name = "Richness (species ha<sup>-1</sup>)", #expression("Richness (species " * ha^-1 * ")"),
      breaks = seq(0, max(1, max_upper), by = 5)
    )
  ) +
  scale_x_reverse(
    labels = function(x) paste0(abs(x), "°S")
  ...
  theme(
    ...
    axis.title = element_markdown(size = rel(0.9))
  ) +
  labs(x = "Latitude") +
  facet_wrap(~model, nrow = 1, axes = "all_y", axis.labels = "margins")

# Display the plot
print(plot_latitude_relationship_sup)
