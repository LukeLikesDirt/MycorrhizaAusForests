...
##
## First, I will subset samples by seuqencing run prior to denoising with
## DADA2 because:
...
##
## NOTES:
## Data within the subdirectories 'bpa_de0d5f3a_20230917T0915' and
## 'bpa_de0d5f3a_20230917T0916' were downloaded from the AMI website on
## 16/09/2023. The 'bpa_de0d5f3a_20230917T0915' subdirectory contains all soil
## 16S test samples from the AMI website, and the 'bpa_de0d5f3a_20230917T0916'
## subdirectory contains the control samples. Data within the subdirectories
## 'bpa_76ac789f_20230717T0504' and 'bpa_761e3497_20230717T0504' are from a
## spcific Govenrment of Victoria project and are not yet avialble on the AMI
## website but was provided to me by the state government for this project.
## There are some control samples from the Marine Microbiome project here that 
## I will discard.
...
bind_rows(data1, data2, data3, data4) %>%
    as_tibble() %>%
    print(n = Inf)
## There are 43 unique runs

## Most of these data did not come with their control sample
## Here I match runs with control samples, which were provided as a separate file

## Load control sample data
data5 = read.csv(file.path(path, '/bpa_7010ec35_20230925T0937/package_metadata/package_metadata_bpa_7010ec35_20230925T0937_amdb-genomics-amplicon-control.csv'), header = T) %>% 
    select(title) %>%
    mutate(run_ID = as.character(gsub('Australian Microbiome Amplicons Control 16S ', '', title))) %>%
    select(run_ID) %>%
    unique() %>%
    as_tibble()
data6 = read.csv(file.path(path, '/bpa_7010ec35_20230925T0937/package_metadata/package_metadata_bpa_7010ec35_20230925T0937_base-genomics-amplicon-control.csv'), header = T) %>% 
    select(title) %>%
...
## I suppose the other 5 runs did not have controls.
...
path=/data/group/frankslab/project/LFlorence/MycorrhizasAustralianForests/data/AusMicrobiome/16S    # Path to main data directory
mkdir $path/01.Raw_data    # Path to send raw data
raw_data=$path/01.Raw_data    # Define path raw data subdirectory
mkdir $raw_data/run{1..43}    # Make subdirectories for each run
...
for f in $path/bpa_de0d5f3a_20230917T0916/*fastq.gz; do mv $f $raw_data
done
## !!!NOTE!!!
...
for f in $raw_data/*; do
  case "$f" in
    ...
    *J6KGL*) mv "$f" "$raw_data/run2" ;;
    *J7C52*) mv "$f" "$raw_data/run3" ;;
    *J7B92*) mv "$f" "$raw_data/run4" ;;
    *J7LRK*) mv "$f" "$raw_data/run5" ;;
    *K39YG*) mv "$f" "$raw_data/run6" ;;
    *KD7T7*) mv "$f" "$raw_data/run7" ;;
    *JC8RL*) mv "$f" "$raw_data/run8" ;;
    *J8GW4*) mv "$f" "$raw_data/run9" ;;
    *JK5PT*) mv "$f" "$raw_data/run10" ;;
    *K3425*) mv "$f" "$raw_data/run11" ;;
    *K34JG*) mv "$f" "$raw_data/run12" ;;
    *L5K2P*) mv "$f" "$raw_data/run13" ;;
    *KG38F*) mv "$f" "$raw_data/run14" ;;
    *KFPWM*) mv "$f" "$raw_data/run15" ;;
    *A815D*) mv "$f" "$raw_data/run16" ;;
    *AC9BY*) mv "$f" "$raw_data/run17" ;;
    *A819B*) mv "$f" "$raw_data/run18" ;;
    *A5YK6*) mv "$f" "$raw_data/run19" ;;
    *AC9E5*) mv "$f" "$raw_data/run20" ;;
    *B3C7L*) mv "$f" "$raw_data/run21" ;;
    *AGBMG*) mv "$f" "$raw_data/run22" ;;
    *A801W*) mv "$f" "$raw_data/run23" ;;
    *AAHKD*) mv "$f" "$raw_data/run24" ;;
    *AAF2V*) mv "$f" "$raw_data/run25" ;;
    *A815N*) mv "$f" "$raw_data/run26" ;;
    *AGEFE*) mv "$f" "$raw_data/run27" ;;
    *AGEK1*) mv "$f" "$raw_data/run28" ;;
    *AGEJK*) mv "$f" "$raw_data/run29" ;;
    *B3BDY*) mv "$f" "$raw_data/run30" ;;
    *BDGJV*) mv "$f" "$raw_data/run31" ;;
    *BDC3D*) mv "$f" "$raw_data/run32" ;;
    *B3L2P*) mv "$f" "$raw_data/run33" ;;
    *BM3J6*) mv "$f" "$raw_data/run34" ;;
    *BWYGW*) mv "$f" "$raw_data/run35" ;;
    *C27WK*) mv "$f" "$raw_data/run36" ;;
    *C444T*) mv "$f" "$raw_data/run37" ;;
    *C5WDL*) mv "$f" "$raw_data/run38" ;;
    *C3WMR*) mv "$f" "$raw_data/run39" ;;
    *A5YJC*) mv "$f" "$raw_data/run40" ;;
    *A8190*) mv "$f" "$raw_data/run41" ;;
    *A5K1H*) mv "$f" "$raw_data/run42" ;;
    *KN482*) mv "$f" "$raw_data/run43" ;;
    *KN48L*) mv "$f" "$raw_data/run44" ;;
  esac
done
...
for run_dir in $raw_data/run{1..44}; do
  for prefix in 'NEG_1_16S_AGRF_GGAGACAAGGGA_' 'NEG1_16S_AGRF_' 'NEG2_16S_AGRF_' 'Neg1_16S_AGRF_' 'Neg2_16S_AGRF_' 'No_Template_Control_16S_'; do
    for f in "$run_dir"/*"$prefix"*; do
      if [ -e "$f" ]; then
...
## which indicates failed sequencing. I will remove these files too using a
## threshold of 100kb
...
## The samples are now ready for DADA2.