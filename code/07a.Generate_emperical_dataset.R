...
native_species <- fread("generated_data/global_tree_mycorrhizal_types.txt") %>%
  filter(native_status == "native", mycorrhizal_type != "uncertain") %>%
...
fungal_root_emp <- fread("generated_data/emperical_measurments.txt") %>%
  filter(
    ...
    # Remove mycorrhizal type errors
    !mycorrhizal_type == "error"
    ) %>%
  select(
...
niche_breadth <- fread("generated_data/niche_estimates.txt")
...
dual_genera <- fread("generated_data/global_tree_mycorrhizal_types.txt") %>%
  filter(native_status == "native", mycorrhizal_type != "uncertain") %>%
...
cat("Total genra classified:", fungal_root_curated %>% pull(genus) %>% unique() %>% length(), "\n")
cat("Total families classified:", fungal_root_curated %>% pull(family) %>% unique() %>% length(), "\n\n")
...
# Compute genus-level consensus for mycorrhizal types within genera
genus_consensus <- fungal_root_curated %>%
  group_by(genus) %>%
  mutate(
    total_species = n()
  ) %>%
  ...
  summarise(
    type_count = n(),
    total_species = first(total_species),
    .groups = "drop"
  ) %>%
  arrange(desc(type_count)) %>%
  mutate(
    type_proportion = type_count / total_species
  ) %>%
  group_by(genus) %>%
  slice(1) %>%
  select(genus, consensus_mycorrhizal_type = mycorrhizal_type,consensus_type_proportion = type_proportion) %>%
  ungroup()

# Summerise mycorrhizal types by family and genus
cat("\n=== MYCORRHIZAL TYPE SUMMARY BY GENUS ===\n\n")
genus_type_table <- fungal_root_curated %>%
  group_by(genus, mycorrhizal_type) %>%
  summarise(
    family = first(family),
    count = n(),
  ...
  select(family, genus, AM, EcM, Dual = `EcM-AM`, NM) %>%
...
# Read in the FungalRoot genus estimates
funagl_root <- fread("data/FungalRoot/harmonised_fungal_root.txt") %>%
  select(genus, mycorrhizal_type) %>%
  mutate(
    # Mutate Eucalypts to EcM
    mycorrhizal_type = case_when(
      genus %in% c("Angophora", "Corymbia", "Eucalyptus") ~ "EcM",
      TRUE ~ mycorrhizal_type
      ),
    # Mutate faculative AM (NM-AM) to AM
    mycorrhizal_type = recode(mycorrhizal_type, "NM-AM" = "AM")
    ) %>%
  unique()

# See is Fungal root and empirical estimates match at the genus level
genus_comparison <- genus_consensus %>%
  left_join(funagl_root, by = "genus") %>%
  select(genus, consensus_mycorrhizal_type, fungal_root_mycorrhizal_type = mycorrhizal_type) %>%
  mutate(match = consensus_mycorrhizal_type == fungal_root_mycorrhizal_type)
...
fwrite(
  inner_join(
    ...
    # Add the FungalRoot genus-level estimates for comparison
    left_join(
      genus_comparison %>%
        select(genus, fungal_root_mycorrhizal_type, match),
      by = "genus") %>%
    select(
      family, genus, AM, EcM, Dual, NM, consensus_mycorrhizal_type, 
      consensus_type_proportion, fungal_root_mycorrhizal_type, match
    ) %>%
